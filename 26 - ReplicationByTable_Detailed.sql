...
WHERE 
    ...
    --    )
       --and 
     --  da.[name] = 'DWPPRHWSQLI05-mops-MOPSMI-DWPPRHWSQL01-3'
    dh.[runstatus] <> 1
...
SELECT [Publisher]
    ...
    ,SUM([DelivCmdsInDistDB]) AS [DelivCmdsInDistDB]
FROM #ReplStats
where [CurrentSessionDeliveryRate] > 0
GROUP BY [Publisher]
    ,[PublicationName]
    ,[PublisherDB]
    ,[Article]
    ...
    ,SUM([DelivCmdsInDistDB]) AS [DelivCmdsInDistDB] from #ReplStats
       where [CurrentSessionDeliveryRate] > 0
